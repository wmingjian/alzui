...
function __newClass(){
	...
	return function(){  //"F" + new Date().getTime()
		...
		//仅仅是一个参数。
		if(arguments.length != 0){  //如果参数个数不为0，执行 create 方法
			if(this.create)  //this._className.indexOf("alz.ui.") == 0
...
function __contextImp(){
	...
	this._class = function(className, superClass, classImp){
		...
		clazz._super = superClass[__proto];  //绑定 _super 属性到类上面（Object.prototype）
		...
		__inDeclare = false;
		clazz[__proto].constructor = clazz;
		clazz[__proto]._className = className;
		clazz[__proto]._clazz = clazz;
		clazz[__proto]._exts = [];  //类的扩展代码
		...
		if(typeof classImp == "function"){
			classImp.apply(clazz[__proto], [clazz._super]);  //function(_super){};
		}
		clazz = null;
	};
...
/**
 * 用于创建核心 Context 实例，即第一个实例
 */
function __createContext(name/*, libs*/){
	var cxt = new Context(name, null);
	//cxt.__name = name;
	//cxt.runtime = null;
	//引入 Context 类，保证以后使用和其他类一样是规范的
	//[TODO]禁止其他lib文件引入Context类，可以防止Context类被滥用
	cxt.alz.Context = Context;
	cxt.Context = Context;
	//runtime._contextList[name] = cxt;  //注册上下文环境
	return cxt;
}

//_class("Context", null, __contextImp);  //标准的_class声明形式，不过现在还不可用
var Context = __newClass();  //这其实是一个类定义
var _p = Context[__proto];
//__contextImp.apply(_p);  //实现原型上的方法
_p.__apply__ = __contextImp;
_p.__apply__();
delete _p.__apply__;
delete _p;
/*#file end*/

with(__createContext("__init__")){
...
_class("WebAppRuntime", "", function(_super){
	...
	this.init = function(){
		var _this = this;
		function __eventHandle(ev){_this.eventHandle(ev || _this._win.event);}
		...
		if(this._config["skin"])
			this._doc.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + this.pathLib + "skin/" + this._config["skin"] + "/skin.css\" />");
	...
	this._checkHostEnv = function(){
		...
		if(this._domScript.src.indexOf(__context__.__name) == -1){
			while(true){
		...
		var keys = [
			"src",
			"pathlib", "pathapp", "pathcss", "pathimg", "pathplugin",
			"css", "lib", "plugin",
		...
		this.pathApp = url.substr(0, url.lastIndexOf("/") + 1);
	...
	this.dynamicLoadFile = function(ext, path, files){
		for(var i = 0, len = files.length; i < len; i++){
			...
			if(ext == "css"){
				...
				script.src = url;  //this.getUrlByName(libName)
				this._domScript.parentNode.appendChild(script, this._domScript);
				script = null;
			}
		}
	};
	...
	this.onLibLoad = function(libName){
		...
		if(lib){
			...
			//初始化所有应用
			for(var i = 0, len = this._apps.length; i < len; i++){
				if(this._apps[i].init){
					this._apps[i].init();
				}
			}
			for(var i = 0, len = this._apps.length; i < len; i++){
				this._apps[i].onContentLoad();
			}
	...
	this.createApp = function(appClassName, parentApp, len){
		/*
		if(this._apps.length >= 1){
			this._win.alert("简洁版只能创建一个应用");
			return;
		}
		*/
		//需要保证多于一个参数，阻止create方法自动执行
		var app = new alz[appClassName](parentApp, len);
		//app.init();
		this._apps.push(app);
		return app;
	};
	this.getAppByClassName = function(className){
		for(var i = 0, len = this._apps.length; i < len; i++){
			if(this._apps[i]._className == className)
				return this._apps[i];
		}
		return null;
	};
...
/*#file end*/
	...
	var rt = new alz.WebAppRuntime();
	runtime = rt;
	rt._contextList[__context__.__name] = __context__;
	rt.setContext(__context__);
	rt.init();
	if(rt.getDocument().body) rt.onContentLoaded();
	rt = null;
