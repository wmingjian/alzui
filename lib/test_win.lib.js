...
function getHostenv(nav){
	...
	var re = {
		"opera" : /Opera[\/ ](\d+(?:\.\d+)+)/,  //Opera/9.10,Opera 8.00
		"ie"    : /MSIE (\d+(?:\.\d+)+)/,       //MSIE 6.0
	...
	for(var k in re){
		...
		if(arr){  //re[k].test(nav.userAgent)
			host.env = k == "mf" ? "ff": k;
			host.ver = arr[1];
			host.compatMode = host.env == "ie" && _doc.compatMode == "BackCompat" ? "BackCompat" : "CSS1Compat";
			break;
		}
	}
	return host;
}
runtime._host = getHostenv(runtime._win.navigator);
...
_class("DomUtil", "", function(_super){
	this._init = function(){
		this._nodes = [];
		this._css = null;
		this._css = this.parseCss(_doc.styleSheets[0].rules || _doc.styleSheets[0].cssRules);
	};
	...
	this.getPos = function(ev, type, refElement){
		var pos = type == 1 ? (
			runtime._host.env == "ie"
			? {x: ev.offsetX, y: ev.offsetY}
			: {x: ev.layerX, y: ev.layerY}
		) : {x: 0, y: 0};
		refElement = refElement || _doc.body;
	...
	this.getStyle = function(el){
		...
		if(_doc.defaultView && _doc.defaultView.getComputedStyle)
			style = _doc.defaultView.getComputedStyle(el, null);
	...
	this.getPropertyValue = function(style, name){
		return runtime._host.env == "ie" ? style[name] : style.getPropertyValue(name);
	};
	...
	this.getHeight = function(el){
		...
		//if(!("_height" in obj)){
			if(runtime._host.compatMode != "BackCompat")
				...
				obj._height = _domutil.getStyleProperty(el, "borderTopWidth")
					+ el.offsetHeight  //_domutil.getStyleProperty(el, "height")
					+ _domutil.getStyleProperty(el, "borderBottomWidth");
			}
		//}
		return obj._height;
		//obj = null;
	...
	this.setWidth = function(el, v){
		...
		for(var i = 0, len = arr.length; i < len; i++){
			if(!("_" + arr[i] in obj)){
				obj["_" + arr[i]] = _domutil.getStyleProperty(el, arr[i]);
			}
		}
	...
	this.getInnerWidth = function(el, v){
		...
		obj = null;
		if(isNaN(innerWidth)) _alert("DomUtil::getInnerWidth isNaN(innerWidth)");
		return innerWidth;
	};
	this.getInnerHeight = function(el, v){
		...
		obj = null;
		if(isNaN(innerHeight)) _alert("DomUtil::getInnerHeight isNaN(innerHeight)");
		return innerHeight;
	};
