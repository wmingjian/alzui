...
var __version = "0.0.3"; //当前版本信息，格式:"主版本号.副版本号.修订版本号"
	...
	this.dynamicLoadFile = function(ext, path, files){
		for(var i = 0, len = files.length; i < len; i++){
			...
			if(ext == "css"){
				...
				loader.load(url);  //this.getUrlByName(libName)
				loader = null;
			}
		}
	};
		...
		if(!this.inGadget){
			try{
				while(win.opener != null){
					...
					if(!this._testCrossDomainWindow(w, 3)){  //测试location属性访问
						return win;
					}
					win = w;
				}
