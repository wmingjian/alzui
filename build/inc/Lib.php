...
class LibTool{
	...
	function create_html_single($f){
		...
		for($i = 0, $len = count($arr); $i < $len; $i++){
			$css[] = $this->create_css1($fw_conf->pathcss . $arr[$i], "/framework/res/images/");
		}
		$arr = explode(',', $app_conf->css);
		for($i = 0, $len = count($arr); $i < $len; $i++){
			$css[] = $this->create_css1("/apps/$app_name/" . $app_conf->pathcss . $arr[$i], "res/images/");
		}
...
<body>
	...
	function create_css1($f, $pathimg){
	...
	function create_css($f){
		global $G_cfg, $path_base, $path_cache, $path_css, $map;
		$f = strtolower(str_replace('/alzui-X201', '', $f));
		$path_css = $path_base . dirname($f) . '/';
		$filename = $path_base . $f;
		$code = file_get_contents($filename);
		$code = preg_replace_callback('/\n@import url\(\"([\w\-]+\.css)\"\);/', array($this, "do_import_css"), $code);
		if($G_cfg['autogen']){
			file_put_contents($path_cache . basename($f), $code);
		}
		return '/* auto generated by css.php */'
			. "\n"
			. $code;
	}
	...
	*/
	...
	function do_import_css($matches){
		global $path_css;
		$name = $matches[1];
		$code = $this->loadCssFile($path_css . $name);
		return "\n\n/* file=\"" . $name . "\" */\n"
			. ($code === null ? "" : $code);
	}
